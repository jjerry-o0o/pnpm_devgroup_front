...
const DetailInfo = () => {
  ...
  const toggleLiked = () => {
    ...
    if (oldLikedList?.includes(id) || !oldLikedList) {
      oldLikedList?.forEach((item: string) => {
  ...
  return (
    <div className="">
        ...
        <div className="flex flex-col max-w-[1320px] w-full">
          <div className="flex pt-8">
            {repositoryDetailInfo?.thumbnailPath || repositoryDetailInfo?.thumbnailWebLink ? (
              <img
                src={repositoryDetailInfo?.thumbnailPath ?? repositoryDetailInfo?.thumbnailWebLink}
                alt={repositoryDetailInfo?.title}
            ...
            <div className="flex flex-col ml-4">
              ...
              <div className="flex">
                ...
                >
                  {repositoryDetailInfo?.url}
                  <TbExternalLink className="ml-1" />
                </button>
              </div>
              <span className="flex items-center text-[16px]">
                <FaInfoCircle className="mr-4 text-[#136ffe] " />
                {repositoryDetailInfo?.introduction}
              </span>
            </div>
          </div>
          <div className="flex border rounded-xl bg-background w-full p-6 mb-8 justify-between">
            <div className="flex gap-4">
              <div className="flex items-center bg-[var(--bg-gray)] w-fit gap-2 p-2">
                <GoHeartFill size="14" color="#dc3545" />
                <span className="font-[600]">{repositoryDetailInfo?.likeCount}</span> likes
              </div>
              <div className="flex items-center bg-[var(--bg-gray)] w-fit gap-2 p-2">
                <IoEyeSharp size="15" color="#136ffe" />
                <span className="font-[600]">{repositoryDetailInfo?.clickCount}</span> views
              </div>
            </div>
            <div className="flex items-center bg-[var(--bg-gray)] w-fit gap-2 p-2">
              <Clock size="15" color="#136ffe" />
              {repositoryDetailInfo?.updatedAt}
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="flex flex-col items-center bg-background border-t px-[67px] pt-8">
        <div className="flex flex-col max-w-[1320px] w-full">
          <span className="flex items-center text-[26px] font-[600]">
            <IoDocumentText color="#136ffe" size="30" className="mr-2" />
            Overview
          </span>
          ...
          <div className="flex justify-center gap-3 border bg-[var(--bg-gray)] py-8 mb-6">
            ...
            >
              ...
              <button type="button" id="likedBtn" onClick={() => id && toggleLiked()}>
                {likedStatus ? 'Liked' : 'Like'}
              </button>
            </label>
          </div>
          <div className="border bg-background h-fit p-[32px] mb-[64px]">{repositoryDetailInfo?.description}</div>
        </div>
      </section>
    </div>
  );
